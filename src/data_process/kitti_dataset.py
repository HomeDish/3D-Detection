...
class KittiDataset(Dataset):
    ...
    def load_img_only(self, index):
        ...
        # print('sample id :{:06d}'.format(sample_id))
        ### end add
    ...
    #     lidar_file = os.path.join(self.lidar_dir, '{:06d}.bin'.format(idx))
    #     assert os.path.isfile(lidar_file)
    #     data = np.fromfile(lidar_file, dtype=np.float32).reshape(-1, 4)
    #     # print(type(data),data.shape)
    #     indexs = np.sort(np.random.randint(0,data.shape[0],int(data.shape[0]/4)))
    #     data = data[indexs,:]
    #     return data
    def get_lidar(self, idx):
        if os.path.exists(os.path.join(self.lidar_dir, '{:06d}.csv'.format(idx))):
            lidar_file = os.path.join(self.lidar_dir, '{:06d}.csv'.format(idx))
            data = pd.read_csv(lidar_file, header=None)
            data = np.array(data).reshape(-1, 4).astype(np.float32)
            ### inverse x-axis data by liujialai
            # inv_matrix = np.array([-1,1,-1,1]*data.shape[0]).reshape(-1,4);data = inv_matrix*data
            ### end inverse
        else:
            lidar_file = os.path.join(self.lidar_dir, '{:06d}.bin'.format(idx))
            data = np.fromfile(lidar_file, dtype=np.float32).reshape(-1, 4)
